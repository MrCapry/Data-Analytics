...
-- To_Char converts in char (number of date)
  -- RR-სა და YY-ს შორის სხვაობას საუკუნე იწვევს
SELECT 
to_char(23),
cast('23' as number),
to_char(a.sales_units),
to_char(sales_date,'day-mm-yyyy'), -- ფორმატის მიცემაც მოსულა
to_char(sales_date,'yy'),
to_char(sales_date,'RR'), -- საუკუნის და Current year-ის მიხედვით არის სხვაობა , RR-სა და YY-ის შორის
to_char(add_months(SYSDATE,-500),'RR'), -- XX საკუნის თემაში და Current Year-ის
...
-- NLS-ში შეიძლება Local Currency-ის ცვლილება
...
-- TO_NUMBER
-- მეორე პარამეტრით უთითებ, რა ფორმაში გაქვს მიღებული Char, რათა სწორად გარდაქმნა


SELECT
sales_date,
TO_NUMBER('$6,125.21','$99,999.99'),
 -- TO_NUMBER('$6,125.21','99,999.99') ეს არ მოსულა
TO_DATE('Jun 12,2005','Mon DD, YYYY') -- ამის ფორმატიც უნდა ემთხვეოდეს
FROM sales
where sales_date > to_date('15-May-2024','DD-Mon-YYYY'); -- ასეც მოსულა, ამის გამოყენება
...
sales_date,
order_date,
NVL(order_date,sysdate) -- იგივე Data Type უნდა მიაწოდო
FROM sales;
...
from employees
WHERE NULLIF(length(first_name),length(last_name)) IS NULL; -- მაგალითი ამ ფუნქციისთვის
